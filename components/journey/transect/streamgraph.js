...
export function PlotTransectLayers(
  data,
  {
    width,
    height,
...
export default function Streamgraph(
  ...
  if (risk == 'all') {
    ...
    const xCenter = xScale(
      journeyFocusData[0].x2 + (journeyFocusData[1].x1 - journeyFocusData[0].x2) / 2
    );
    const xOffset = 64;
...
export function DrawTooltip(config) {
  ...
  const createTooltip = d3.select('#journey').append('div').attr('id', 'transectTooltip');
