...
export default function CityTip({ hoverInfo, data }) {
  ...
  const countryText = `${Math.floor(
    (nationalMigrantCount * 100) / totalSurveyed
  )}% of all migrants surveyed in Libya come from`;
...
function InfoBox({ percentage, text, region, small, bold }) {
  return (
    <div className={styles.infoBox}>
      {percentage && (
        <h4 style={{ ['--weight' as any]: bold ? 'bold' : 'initial' }}>{percentage}%</h4>
      )}
      <p style={{ ['--size' as any]: small ? '12px' : '0.875rem' }}>
        {text} <span style={{ fontWeight: 'bold' }}>{region}</span>{' '}
      </p>
    </div>
  );
}
