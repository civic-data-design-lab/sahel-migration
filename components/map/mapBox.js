...
export default function MapBox({ activeSource, risks, tipData, toggleMap }) {
  ...
  const [mapStyle, setMapStyle] = useState(
    'mapbox://styles/mitcivicdata/cld132ji3001h01rn1jxjlyt4'
  );
  ...
  useEffect(() => {
    ...
    if (mapRef.current) {
      ...
      mapRef.current.on('click', 'migration', () => toggleMap());
    }
  }, [activeSource]);
  return (
    <RouteContext.Provider value={routeValue}>
      <div className={styles.mapContainer}>
        ...
        >
          ...
          {selectedSegment && activeSource === 'extremeHeat' && <RouteTip hoverInfo={routeInfo} />}
          ...
          <Layer
            {...layersObject['migrationRouteStyle']}
            lineJoin="round"
            paint={{
